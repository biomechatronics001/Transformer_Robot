...
int num_nodes = 24;
unsigned long time_period = 500; //unit ms sample rate=1/time_period
String ID_name = "Master"; //change the name
int ID = 0; //48-30
int Ledstatus = 0;
int packageLength = 30;
int rs232Length = 32;
char data_actual_angle[31] = {0};
char data_command[31] = {0};
char data_actual_command[31] = {0};
char data_previous_command[31] = {180};
...
//    {
//      data_actual_command[i] = data_command[i];
//    }
//    if ( Bluefruit.Central.connected() && clientUart.discovered() )
//    {
//      clientUart.write(&data_actual_command[0], packageLength);
//    }
//  }
//
...
//      {
//        Serial.readBytes(&data_command[0], packageLength);
//      }
//    }
//  }
//}
...
void loop()
{
  ...
  if ((cur_time - pre_time) > time_period){
    pre_time = cur_time;
    Serial.write(data_tx_rs232, rs232Length);
    for (int stepCounter = 0; stepCounter <= 180; stepCounter++){
      for (int i = 0; i < num_nodes; i++){
        if (data_command[i] == data_previous_command[i]){
          data_actual_command[i] = data_command[i];
        } else if (data_command[i] > data_previous_command[i]){
          data_actual_command[i] = floor(data_previous_command[i] + (data_command[i] - data_previous_command[i]) / rotationStep * stepCounter);
        } else if (data_command[i] < data_previous_command[i]){
          data_actual_command[i] = floor(data_previous_command[i] - (data_previous_command[i] - data_command[i]) / rotationStep * stepCounter);
        }
      }
      if ( Bluefruit.Central.connected() && clientUart.discovered() ){
      clientUart.write(&data_actual_command[0], packageLength);
      }
      delay(rotationDuration / rotationStep);
    }
    for (int i = 0; i < num_nodes; i++){
      data_previous_command[i] = data_command[i];
    }
...
void scan_callback(ble_gap_evt_adv_report_t* report)
{
  ...
  {
    if (buffer[0] == 'I' && buffer[1] == 'D' && buffer[2] == (ID + 48))
    {
...
void cent_bleuart_rx_callback(BLEClientUart& cent_uart)
{
  ...
  if (length_rx_cent >= packageLength)
  {
    ...
    {
      data_actual_angle[i] = data_tx_rs232[i + 2];
    }
    //    Serial.print("Length: ");
